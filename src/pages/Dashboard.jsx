...
export default function Dashboard() {
  ...
  const stats = [
    { label: "Today's Orders", value: "42", icon: <ShoppingBag size={24} strokeWidth={2.5} color="var(--accent)" />, bgGlow: "var(--accent-glow)" },
    { label: "Revenue", value: "₹8,450", icon: <IndianRupee size={24} strokeWidth={2.5} color="var(--success)" />, bgGlow: "rgba(34, 197, 94, 0.15)" },
    { label: "Pending Orders", value: "5", icon: <Clock size={24} strokeWidth={2.5} color="#3b82f6" />, bgGlow: "rgba(59, 130, 246, 0.15)" }
  ];

  return (
    <div style={{ position: "relative", minHeight: "100%", paddingBottom: "2rem" }}>
      <div style={{ position: "absolute", top: -40, left: -32, right: -32, bottom: -40, backgroundImage: `url("${doodlePattern}")`, backgroundRepeat: "repeat", backgroundSize: "160px", opacity: 0.8, zIndex: 0, pointerEvents: "none" }} />
      <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div style={{ marginBottom: "2.5rem", display: "flex", flexDirection: "column", gap: 16 }}>
          <h1 style={{ fontFamily: "'Syne',sans-serif", fontSize: isMobile ? "2rem" : "2.6rem", fontWeight: 800, color: "var(--text)", lineHeight: 1.1 }}>
            Welcome back, <br /><span style={{ color: "var(--accent)" }}>Vendor</span>
          </h1>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "color-mix(in srgb, var(--surface) 85%, transparent)", backdropFilter: "blur(12px)", border: "1.5px solid var(--border)", padding: "8px 16px", borderRadius: 12, boxShadow: "var(--shadow2)" }}>
              <CalendarDays size={18} strokeWidth={2.5} color="var(--accent)" />
              <span style={{ fontSize: "0.95rem", fontWeight: 700, color: "var(--text2)" }}>{formattedDate}</span>
            </div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "color-mix(in srgb, var(--surface) 85%, transparent)", backdropFilter: "blur(12px)", border: "1.5px solid var(--border)", padding: "8px 16px", borderRadius: 12, boxShadow: "var(--shadow2)" }}>
              <Clock size={18} strokeWidth={2.5} color="var(--accent)" />
              <span style={{ fontSize: "0.95rem", fontWeight: 700, color: "var(--text2)" }}>{formattedTime}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fit, minmax(300px, 1fr))", gap: 24 }}>
          {stats.map((stat, i) => (
            <div key={i} style={{ background: "color-mix(in srgb, var(--surface) 85%, transparent)", backdropFilter: "blur(16px)", border: "1.5px solid color-mix(in srgb, var(--border) 60%, transparent)", borderRadius: 20, padding: "24px", display: "flex", flexDirection: "column", gap: 20, boxShadow: "var(--shadow)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ color: "var(--text3)", fontSize: "0.95rem", fontWeight: 700, textTransform: "uppercase" }}>{stat.label}</div>
                <div style={{ width: 48, height: 48, borderRadius: 14, background: stat.bgGlow, display: "flex", alignItems: "center", justifyContent: "center" }}>{stat.icon}</div>
              </div>
              <div style={{ fontSize: "2.8rem", fontWeight: 800, fontFamily: "'Syne',sans-serif", color: "var(--text)" }}>{stat.value}</div>
            </div>
          ))}
        </div>
      </div>
